...
module.exports.run = async function({ api, event, Users  , Threads}) {
  ...
  ///////////////////////////////////////////////////////
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "lỏd" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
    var mlg="Kết nối thành công\nĐã load toàn bộ lệnh và người dùng trong nhóm.\n❌ Nếu nhóm của bạn chưa kích hoạt sử dụng bot, vui lòng sử dụng lệnh 'callad' để liên hệ Admin.\n─────────────────\n🌐 Facebook: https://www.facebook.com/591868507"
    	return api.sendMessage(threadID,async () => {
await api.shareContact(`${mlg}`, 591868507, threadID);
});
